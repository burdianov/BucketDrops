...
public class Divider extends RecyclerView.ItemDecoration {
    ...
    public Divider(Context context, int orientation) {
        ...
        if (orientation != LinearLayoutManager.VERTICAL) {
            throw new IllegalArgumentException("This Item Decoration can be used " +
                    "only with a RecyclerView that uses a LinearLayoutManager " +
    ...
    private void drawHorizontalDivider(Canvas c, RecyclerView parent,
        ...
        for (int i = 0; i < count; i++) {
            if (AdapterDrops.FOOTER != parent.getAdapter().getItemViewType(i)) {
                ...
                mDivider.draw(c);
                Log.d(TAG, "drawHorizontalDivider: "
